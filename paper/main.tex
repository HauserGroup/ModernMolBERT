...
\usepackage[colorlinks=true,
            ...
            pdfkeywords={molecular language model, SELFIES, ModernBERT,
                         representation learning, cheminformatics, APE tokenizer},
...
\selfies{}-aware \emph{Atom Pair Encoding} (\ape{}) tokenizer.
...
weights, the \ape{} tokenizer, and training code.
\end{abstract}
...
Molecular representation learning is therefore an essential preprocessing step. A useful molecular embedder should preserve chemically meaningful structure, avoid artefacts introduced by molecular notation, and be efficient enough to apply across large compound collections. ModernMolBERT addresses all of these by combining chemically valid SELFIES strings, a SELFIES-aware Atom Pair Encoding tokenizer, and an efficient ModernBERT encoder trained as a reusable frozen molecular embedder.

\paragraph{From fingerprints to learned representations.}
...
\paragraph{Molecular language models on \smiles{}.}
The BERT~\citep{devlinBERTPreTrainingDeep2019} family of bidirectional
encoder models revolutonized how masked language model~(MLM) pretraining on
large unlabelled corpora could produce representations that transfer to many
downstream tasks with minimal fine-tuning.
Molecules encoded as \smiles{} (Simplified Molecular-Input Line-Entry System)
strings~\citep{weininger1988smiles} which are a linear notation that encodes
atomic connectivity as a depth-first traversal of the molecular graph. Representations that
are a natural substrate for words and sentences.
...
established that MLM pretraining on \smiles{} yielde representations
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{l l l l l}
    ...
    \midrule
    ChemBERTa~\citep{chithranandaChemBERTaLargeScaleSelf2020}
      & \smiles{}  & BPE      & PubChem      & RoBERTa      \\
    ChemBERTa-2~\citep{ahmadChemBERTa2TowardsChemical2022}
      & \smiles{}  & BPE/atom & PubChem      & RoBERTa      \\
    MolBERT~\citep{fabianMolecularRepresentationLearning2020}
      & \smiles{}  & atom     & \chembl{}    & BERT         \\
    MoLFormer~\citep{rossLargescaleChemicalLanguage2022}
      & \smiles{}  & atom     & ZINC+PubChem & Linear-attn. \\
    SELFormer~\citep{yukselSELFormerMolecularRepresentation2023}
      & \selfies{} & BPE      & \chembl{}    & RoBERTa      \\
    \midrule
    \textbf{\model-small}
      & \textbf{\selfies{}} & \textbf{\ape{}} & \textbf{\chembl{}~36} & \textbf{\modernbert{}} \\
    \textbf{\model-base}
      & \textbf{\selfies{}} & \textbf{\ape{}} & \textbf{\chembl{}~36} & \textbf{\modernbert{}} \\
    \bottomrule
  \end{tabularx}
  \caption{%
    ...
    \emph{Tokenizer}: tokenization strategy
...
\paragraph{Limitations of \smiles{}.}
...
Second, subword tokenizers such as Byte Pair Encoding~(BPE)~\citep{sennrichNeuralMachineTranslation2016},
...
destroying the chemically meaningful units a molecular tokenizer should
preserve.

\paragraph{Guaranteed chemical validity with \selfies{}.}
...
combination of \selfies{}, a \selfies{}-adapted atom-preserving tokenizer, and
a \modernbert{}-style encoder has not been systematically evaluated as a
molecular embedder.

\paragraph{Atom Pair Encoding.}
Tokenization which is the mapping from a raw string to discrete vocabulary units, is almost as essential as the model itself.
Atom-level tokenization is chemically principled but
...
middle ground: compact like subword tokenization, chemically principled like
atom-level tokenization.

\paragraph{Modernising the encoder architecture.}
Many molecular string encoders such ChemBERTa, ChemBERTa-2, MolBERT,
...
The MLM objective does not specify directly tokens to mask. Standard BERT masks tokens
...
tokenizer. The models are trained from scratch with MLM on 2.4M curated
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    canonicalised, and converted to \selfies{} strings, then pre-tokenized with
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    under perturbation or naive tokenization --- mismatched brackets and
...
holds under arbitrary mutation: a single random bit-flip leaves a \smiles{}
string valid only 26.6\% of the time and ten bit-flips reduce validity to
0.2\%, whereas \selfies{} remains 100\% valid in both cases. The representation
...
The atom-preserving \ape{} tokenizer~\citep{leonComparingSMILESSELFIES2024}
(\cref{sec:ape-tokenizer}) was shown to outperform standard BPE in a controlled
...
\subsection{Atom Pair Encoding Tokenizer}
\label{sec:ape-tokenizer}

We use a custom reimplementation of the \ape{} algorithm
\citep{leonComparingSMILESSELFIES2024,mayuareAPETokenizer} with
several modifications for \selfies{} pretraining at scale.
...
\texttt{min\_freq} is the binding early-stopping criterion. Full tokenizer
statistics are reported in \cref{tab:tokenizer-stats} (\cref{app:tokenizer}).

\paragraph{Greedy longest-match tokenization.}
...
model weights were initialized and trained on the completed 631-token vocabulary
from the outset.
...
configuration: RoPE, GeGLU feed-forward blocks, pre-normalization, no
linear-layer biases. It usesu npadded Flash Attention with global attention
...
compact regime below MoLFormer-XL~\citep{rossLargescaleChemicalLanguage2022}
(44.4M), Chemformer~\citep{irwinChemformerPretrainedTransformer2022} (45M), and
...
lightly sanitized with RDKit~\citep{landrumRDKitOpenSourceCheminformatics} (ring perception and aromaticity assignment
only, avoiding full sanitization), and canonicalized to isomeric
...
Molecules that fail RDKit parsing, limited sanitization,
canonicalization, or \selfies{} encoding are excluded.
...
The resulting \selfies{} strings are subsequently pre-tokenized with the
\ape{} tokenizer~\citep{leonComparingSMILESSELFIES2024} using greedy
longest-match tokenization.
The full curation and pre-tokenization pipeline is available at
...
random forests, and $k$-nearest neighbours --- whose hyperparameters are
...
datasets, and \model{}-small attains 77.3 across all 25 datasets
...
global physicochemical character rather than rare substructure recognition.
...
choices, \ape{} tokenization, and differences in the training recipe; matched
...
in lipophilicity, drug-likeness, polarity, flexibility, aromaticity, and size,
indicating that the embedding space captures global physicochemical structure
even though it is never given these properties as training signal.
...
representation, the \ape{} tokenizer, and the \modernbert{} backbone) are not
...
dedicated tokenizer and representation ablations are left for future work.
...
choices shared by all released models.
...
\ape{} tokenizer. This scope differs from large generative chemical language models,
...
claim about all molecular foundation models.
...
consistent with benefits from the \modernbert{} backbone and \ape{} tokenizer,
...
molecules. The contribution is therefore a compact, reproducible
...
natural products, agrochemicals, or other under-represented chemical
...
models~\citep{winterLearningContinuousDataDriven2019,kovacsSimSonSimpleContrastive2025}.
...
The \selfies{}-adapted \ape{} tokenizer forms merge candidates from
...
non-drug-like chemistry. A second direction is matched single-axis ablations
of the tokenizer and backbone choices---substituting \ape{} with standard BPE
...
\modernbert{} encoder architecture with a \selfies{}-adapted \ape{} tokenizer
...
\ape{} tokenizer can approach fingerprint-level frozen-feature performance while substantially
...
valid by construction, tokenized in chemically meaningful units, and built on
...
Model weights and the \ape{} tokenizer are available on Hugging Face
...
The final tokenizer is trained on 2M randomly sampled \chembl{}~36
...
all model weights were initialized and trained from the outset.
Special tokens are assigned fixed ids: \texttt{[BOS]}\,=\,0,
\texttt{[PAD]}\,=\,1, \texttt{[EOS]}\,=\,2, \texttt{[UNK]}\,=\,3,
\texttt{[MASK]}\,=\,4.
Evaluated on 10\,000 held-out \chembl{}~36 molecules, the tokenizer
