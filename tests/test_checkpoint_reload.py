...
import torch
from transformers import AutoModelForMaskedLM, ModernBertConfig
...
def test_end_to_end_save_and_reload_with_tokenizer_artifacts(tmp_path: Path):
    ...
    reloaded_tokenizer = APETokenizer()
    reloaded_tokenizer.load_vocabulary(str(final_model_dir / "tokenizer.json"))

    eval_batch = reloaded_tokenizer(
        "[C][C][O]", add_special_tokens=True, return_tensors="pt"
    )
    eval_batch = {k: v.unsqueeze(0) for k, v in eval_batch.items()}
    with torch.no_grad():
